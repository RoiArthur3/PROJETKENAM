...
class EmailNotificationService
{
    ...
    public function sendOperationSubmittedNotifications(Operation $operation, $demandeur)
    {
        try {
            ...
            // 4. Email au DG si montant dépasse le seuil défini
            $entreprise = \App\Models\EntrepriseSettings::getActive();
            $seuil = $entreprise->seuil_validation_dg ?? 1000000;
            
            if ($operation->montant >= $seuil) {
                $dgEmail = $entreprise->email_dg ?? config('app.dg_email', 'dg@kenamservices.com');
                try {
                    Mail::to($dgEmail)
                        ->send(new HighAmountOperationNotification($operation, $demandeur));
                    Log::info("Email de montant élevé envoyé au DG: {$dgEmail}");
                } catch (\Exception $e) {
                    Log::warning("Email DG non envoyé: {$dgEmail} - " . $e->getMessage());
                }
            }
