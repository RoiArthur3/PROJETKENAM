...
class ModuleAccessMiddleware
{
    ...
    private array $modulePermissions = [
        ...
        'validations' => 'can_access_operations', // Les validations font partie des opérations pour les agents
        ...
        'tresorerie' => 'can_access_accounting', // La trésorerie utilise les permissions de la comptabilité
        ...
        'magasin' => 'can_access_warehouse', // Le magasin utilise les permissions du warehouse
    ...
    public function handle(Request $request, Closure $next, string $module): Response
    {
        ...
        if (!$user->canAccessModule($module)) {
            ...
            $fallbackUrl = '/operations/dashboard';
            if (method_exists($user, 'getFirstAccessibleModuleUrl')) {
                $candidate = $user->getFirstAccessibleModuleUrl();
                if (is_string($candidate) && $candidate !== '') {
                    $fallbackUrl = $candidate;
                }
            }

            // Éviter une boucle de redirection si la route courante correspond déjà à la destination.
            if ($request->path() === ltrim($fallbackUrl, '/')) {
                $fallbackUrl = '/mon-profil';
            }

            return redirect($fallbackUrl)
                ->with('warning', "Accès refusé au module {$module}");
        }
