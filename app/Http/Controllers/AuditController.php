...
class AuditController extends Controller
{
    ...
    public function index()
    {
        if (!Schema::hasTable('inspections')) {
            $audits = new LengthAwarePaginator([], 0, 10);
    ...
    public function dashboard()
    {
        ...
        // Vérifier si la colonne checkable_type existe avant de l'utiliser
        if (Schema::hasTable('checkings') && Schema::hasColumn('checkings', 'checkable_type')) {
            $controles = $controles->whereHas('checking', function ($query) {
                $query->where('checkable_type', Vehicule::class);
            });
        }

        $controles = $controles->get();
        ...
        if (Schema::hasTable('anomalies')) {
            ...
            // Vérifier si la colonne checkable_type existe avant de l'utiliser
            if (Schema::hasTable('checkings') && Schema::hasColumn('checkings', 'checkable_type')) {
                $alertes = $alertes->whereHas('inspection.checking', function ($query) {
                    $query->where('checkable_type', Vehicule::class);
                });
            }

            $alertes = $alertes->orderBy('severity', 'desc')->get();
        } else {
            $alertes = collect();
        }
