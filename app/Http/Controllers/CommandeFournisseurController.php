...
use App\Exports\CommandesMissionPrestationExport;
...
{
    private const MISSION_SOURCE_CLASS = 'App\\Models\\CommandeFournisseur';
    ...
    public function indexGlobal(Request $request)
    {
        try {
            ...
            $classificationContext = $this->buildClassificationContext();

            $query = $this->buildGlobalCommandesQuery($request, $classificationContext);

            $statsCollection = (clone $query)->get();
            $statsNature = $this->buildNatureStats($statsCollection, $classificationContext);
            $stats = [
                'total' => $statsCollection->count(),
                'total_montant' => (float) $statsCollection->sum('montant_ttc'),
                'validees' => $statsCollection->where('statut', 'validee')->count(),
                'en_attente' => $statsCollection->where('statut', 'en_attente')->count(),
                'missions' => $statsNature['mission'] ?? 0,
                'prestations' => $statsNature['prestation'] ?? 0,
            ];

            $commandes = $query->paginate(15)->withQueryString();
            $this->hydrateCommandesNature($commandes->getCollection(), $classificationContext);

            return view('fournisseurs.commandes', compact('commandes', 'fournisseurs', 'stats'));

        } catch (\Exception $e) {
            Log::error("Erreur globale indexGlobal: " . $e->getMessage());
    ...
    public function exportGlobalExcel(Request $request)
    {
        ...
        $query = $this->buildGlobalCommandesQuery($request, $classificationContext);
        $commandes = $query->get();
        $this->hydrateCommandesNature($commandes, $classificationContext);

        return Excel::download(
            new CommandesMissionPrestationExport($commandes),
            'commandes-fournisseurs-' . now()->format('Ymd_His') . '.xlsx'
        );
    }
    ...
    public function exportGlobalPdf(Request $request)
    {
        ...
        $pdf = DomPdf::loadView('fournisseurs.commandes.export-pdf', [
            'commandes' => $commandes,
            'generatedAt' => now(),
        ]);

        return $pdf->download('commandes-fournisseurs-' . now()->format('Ymd_His') . '.pdf');
    }

    private function buildGlobalCommandesQuery(Request $request, array $classificationContext)
    {
        $query = CommandeFournisseur::query()
            ->with(['fournisseur', 'lignes', 'contrat'])
        ...
        if ($request->filled('periode')) {
            $this->applyPeriodeFilter($query, (string) $request->input('periode'));
        }
        ...
        if ($request->filled('recherche')) {
            ...
            $query->where(function ($subQuery) use ($term): void {
                $subQuery->where('reference', 'like', '%' . $term . '%')
                    ...
                    ->orWhereHas('lignes', function ($lignesQuery) use ($term): void {
                        $lignesQuery->where('designation', 'like', '%' . $term . '%')
                            ->orWhere('description', 'like', '%' . $term . '%');
                    });
            });
        }

        if ($request->filled('base') && in_array($request->input('base'), ['mission', 'prestation'], true)) {
            $this->applyBaseFilter($query, (string) $request->input('base'), $classificationContext);
        }

        return $query;
    }

    private function buildClassificationContext(): array
    {
        $context = [
            'mission_ids' => [],
            'prestation_ids' => [],
        ...
        if (Schema::hasTable('vehicle_missions')) {
            $missionSourceIds = DB::table('vehicle_missions')
                ->whereNotNull('source_id')
                ->where('source_type', self::MISSION_SOURCE_CLASS)
                ->pluck('source_id')
                ->map(static fn ($value) => (int) $value)
                ->all();

            $missionReferences = DB::table('vehicle_missions')
            ...
            $missionIdsByReference = [];
            if (!empty($missionReferences)) {
                $missionIdsByReference = DB::table('commande_fournisseurs')
                    ->whereIn('reference', $missionReferences)
                    ->pluck('id')
                    ->map(static fn ($value) => (int) $value)
                    ->all();
            }

            $context['mission_ids'] = array_values(array_unique(array_merge($missionSourceIds, $missionIdsByReference)));
        }

        if (Schema::hasTable('contrat_fournisseurs') && Schema::hasTable('commande_fournisseurs')) {
            $context['prestation_ids'] = DB::table('commande_fournisseurs')
                ->join('contrat_fournisseurs', 'commande_fournisseurs.contrat_id', '=', 'contrat_fournisseurs.id')
                ->whereIn(DB::raw('LOWER(contrat_fournisseurs.type_contrat)'), ['prestation', 'service'])
                ->pluck('commande_fournisseurs.id')
                ->map(static fn ($value) => (int) $value)
                ->all();
        }
    ...
    private function applyPeriodeFilter($query, string $periode): void
    {
        ...
        if ($periode === 'quarter') {
            $query->whereBetween('date_commande', [now()->startOfQuarter(), now()->endOfQuarter()]);
        }
    }

    private function applyBaseFilter($query, string $base, array $classificationContext): void
    {
        $ids = $base === 'mission'
            ? ($classificationContext['mission_ids'] ?? [])
            : ($classificationContext['prestation_ids'] ?? []);
        ...
        $query->whereIn('commande_fournisseurs.id', $ids);
    }

    private function hydrateCommandesNature(Collection $commandes, array $classificationContext): void
    {
        foreach ($commandes as $commande) {
            $commande->setAttribute('nature_commande', $this->detectCommandeNature($commande, $classificationContext));
        }
    }

    private function buildNatureStats(Collection $commandes, array $classificationContext): array
    {
        $result = ['mission' => 0, 'prestation' => 0, 'autre' => 0];

        foreach ($commandes as $commande) {
            $nature = $this->detectCommandeNature($commande, $classificationContext);
            if (!isset($result[$nature])) {
                $result[$nature] = 0;
            }
    ...
    private function detectCommandeNature(CommandeFournisseur $commande, array $classificationContext): string
    {
        $commandeId = (int) $commande->id;

        if (in_array($commandeId, $classificationContext['mission_ids'] ?? [], true)) {
            return 'mission';
        }

        if (in_array($commandeId, $classificationContext['prestation_ids'] ?? [], true)) {
            return 'prestation';
        }
    ...
    /**
     * Rechercher des engins disponibles pour une mission
     */
    public function searchEngins(Request $request)
    {
        try {
            $search = $request->input('search', '');
            $type = $request->input('type', 'all'); // all, vehicule, equipement

            $query = \App\Models\Vehicule::query();

            if ($type === 'vehicule') {
                $query->where('type', 'vehicule');
            } elseif ($type === 'equipement') {
                $query->where('type', 'equipement');
            }
            ...
            // Filtre pour les engins disponibles (non en mission)
            $query->where('statut', 'disponible')
                  ->orWhere('statut', 'en_stock');

            $engins = $query->limit(20)->get(['id', 'immatriculation', 'marque', 'modele', 'type', 'statut', 'numero_serie']);
    ...
    public function pdf(Fournisseur $fournisseur, CommandeFournisseur $commande)
    {
        ...
        $pdf = DomPdf::loadView('fournisseurs.commandes.pdf', [
            'commande' => $commande,
