...
class FactureFournisseurController extends Controller
{
    ...
    public function index(Request $request, $fournisseurId = null)
    {
        ...
        if ($request->filled('statut')) {
            ...
            if ($statut === 'payee') {
                ...
                $query->where('statut', 'partiellement_payee');
        ...
        // Récupérer les pièces jointes de type facture rattachées au fournisseur/numéro de facture
        $documentsParFacture = collect();
        if (Schema::hasTable('document_fournisseurs') && $factures->count() > 0) {
            $fournisseurIds = $factures->pluck('fournisseur_id')->filter()->unique()->values();

            $documents = DB::table('document_fournisseurs')
            ...
            $documentsParFacture = $factures->getCollection()->mapWithKeys(function ($facture) use ($documents) {
                $items = $documents->filter(function ($doc) use ($facture) {
                    if ((int) $doc->fournisseur_id !== (int) $facture->fournisseur_id) {
                        return false;
                    }

                    $numeroMatch = !empty($doc->numero_document) && $doc->numero_document === $facture->numero_facture;
                    $referenceMatch = !empty($doc->reference) && $doc->reference === $facture->reference;

                    return $numeroMatch || $referenceMatch;
                })->values();

                return [$facture->id => $items];
            });
        }
        ...
        if ($fournisseur) {
            $statsQuery->where('fournisseur_id', $fournisseur->id);
        }

        $stats = [
            'total' => (clone $statsQuery)->count(),
            'montant_total' => (clone $statsQuery)->sum('montant_ttc'),
            'montant_paye' => (clone $statsQuery)->sum('montant_paye'),
            'montant_restant' => (clone $statsQuery)->sum('reste_a_payer'),
            'en_retard' => (clone $statsQuery)
    ...
    public function store(Request $request, Fournisseur $fournisseur)
    {
        ...
        try {
            ...
            foreach ($validated['lignes'] as $ligne) {
                ...
                $quantiteTotaleFacturee = $lignesFactureExistantes->get($ligne['ligne_commande_id'], 0) + $ligne['quantite'];
    ...
    public function payerCaisse(Request $request, Fournisseur $fournisseur, FactureFournisseur $facture)
    {
        ...
        try {
            ...
            $caisse->solde_actuel = (float) (($caisse->solde_actuel ?? 0) - $validated['montant']);
            $caisse->save();

            // Mise à jour de la facture
            $facture->montant_paye += (float) $validated['montant'];
            $facture->reste_a_payer = (float) ($facture->montant_ttc - $facture->montant_paye);
