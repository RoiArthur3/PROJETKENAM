...
class ReportingController extends Controller
{
    ...
    public function financier(Request $request)
    {
        ...
        try {
            // ── Caisses ──
            $caissesQuery = \App\Models\Caisse::query();
            if (Schema::hasColumn('caisses', 'est_active')) {
                $caissesQuery->where('est_active', true);
            }

            $caisses = $caissesQuery->orderBy('nom')->get();

            // Compatibilite schema: certaines bases historiques utilisent `solde` au lieu de `solde_actuel`.
            $soldeColumn = 'solde_actuel';
            if (!Schema::hasColumn('caisses', 'solde_actuel') && Schema::hasColumn('caisses', 'solde')) {
                $soldeColumn = 'solde';
            }
            $solde_total = (float) $caisses->sum($soldeColumn);
            ...
            try {
                ...
                Log::warning('Reporting financier: MouvementCaisse indisponible, fallback vers autres sources. ' . $e->getMessage());
                $mouvements = collect([]);
            }
            ...
            // Fallback historique: si aucun flux trésorerie moderne n'est détecté,
            // utiliser les tables comptables recettes/depenses pour alimenter les KPIs.
            if ((float) $total_entrees === 0.0 && (float) $total_sorties === 0.0) {
                if (Schema::hasTable('recettes')) {
                    $recettesQuery = DB::table('recettes')->whereBetween('date_recette', [$startDate->toDateString(), $endDate->toDateString()]);
                    if ($type_filter !== 'sortie') {
                        $total_entrees = (float) $recettesQuery->sum('montant');
                    }
                }

                if (Schema::hasTable('depenses')) {
                    $depensesHistQuery = DB::table('depenses')->whereBetween('date_depense', [$startDate->toDateString(), $endDate->toDateString()]);
                    if ($type_filter !== 'entree') {
                        $total_sorties = (float) $depensesHistQuery->sum('montant');
                    }
                }
            }
            ...
            // ──────────────────────────────────────────────────────────
            // 6. Dettes fournisseurs (prestations/factures non payées)
            // ──────────────────────────────────────────────────────────
            $dettes_fournisseurs = collect([]);
            $total_dettes_fournisseurs = 0;
            $nb_factures_impayees = 0;

            if (Schema::hasTable('facture_fournisseurs') && Schema::hasTable('fournisseurs')) {
                $dettesQuery = DB::table('facture_fournisseurs as ff')
                    ->leftJoin('fournisseurs as f', 'f.id', '=', 'ff.fournisseur_id')
                    ->whereNull('ff.deleted_at')
                    ->where(function ($q) {
                        $q->where('ff.reste_a_payer', '>', 0)
                          ->orWhereIn('ff.statut', ['non_payee', 'partiellement_payee']);
                    });

                $total_dettes_fournisseurs = (float) (clone $dettesQuery)->sum('ff.reste_a_payer');
                $nb_factures_impayees = (int) (clone $dettesQuery)->count('ff.id');

                $dettes_fournisseurs = (clone $dettesQuery)
                    ->orderByDesc('ff.reste_a_payer')
                    ->limit(20)
                    ->get([
                        'ff.id',
                        'ff.reference',
            ...
            $financial_data = [
                ...
                'dettes_fournisseurs' => $total_dettes_fournisseurs,
            ...
            // Conserver un solde de caisses fiable meme si une partie du reporting echoue.
            $fallbackSoldeTotalCaisses = 0;
            $fallbackTotalEntrees = 0;
            $fallbackTotalSorties = 0;
            try {
                $fallbackQuery = DB::table('caisses');
                if (Schema::hasColumn('caisses', 'est_active')) {
                    $fallbackQuery->where('est_active', 1);
                }
                $fallbackSoldeTotalCaisses = (float) $fallbackQuery->sum('solde_actuel');

                // Fallback KPI: utiliser recettes/depenses sur la periode demandee.
                if (Schema::hasTable('recettes') && Schema::hasColumn('recettes', 'date_recette')) {
                    $recettesQuery = DB::table('recettes')
                        ->whereBetween('date_recette', [$startDate->toDateString(), $endDate->toDateString()]);
                    if ($type_filter !== 'sortie') {
                        $fallbackTotalEntrees = (float) $recettesQuery->sum('montant');
                    }
                }
                ...
                if (Schema::hasTable('depenses') && Schema::hasColumn('depenses', 'date_depense')) {
                    ...
                    if ($type_filter !== 'entree') {
                        $fallbackTotalSorties = (float) $depensesQuery->sum('montant');
                    }
                }
            ...
            return view('reporting.financier', [
                'financial_data' => [
                    'total_entrees' => $fallbackTotalEntrees,
                    'total_sorties' => $fallbackTotalSorties,
                    'solde_net' => $fallbackTotalEntrees - $fallbackTotalSorties,
                    'solde_total_caisses' => $fallbackSoldeTotalCaisses,
                    'nb_transactions' => 0,
                    'nb_entrees' => 0,
                    'nb_depenses' => 0,
                    'nb_virements' => 0,
