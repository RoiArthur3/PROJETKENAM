...
class CommercialController extends Controller
{
    ...
    private function getFirstExistingColumn(string $table, array $columns, ?string $fallback = null): ?string
    {
        $schema = DB::getSchemaBuilder();
    ...
    private function normalizeDashboardRows($rows, string $referenceColumn, string $amountColumn)
    {
        return $rows->map(function ($row) use ($referenceColumn, $amountColumn) {
            $row->reference = $row->{$referenceColumn} ?? null;
            $row->montant = (float) ($row->{$amountColumn} ?? 0);

            if (!empty($row->created_at) && !$row->created_at instanceof Carbon) {
    ...
    private function mapClientFields(array $fields)
    {
        ...
        // Générer un code client unique si le champ 'code' existe et n'est pas déjà fourni
        if ($schema->hasColumn('clients', 'code') && !isset($data['code'])) {
            $data['code'] = $this->generateClientCode();
        }

        // Ajouter les valeurs par défaut pour les champs obligatoires manquants
        if ($schema->hasColumn('clients', 'contact_prenom') && !isset($data['contact_prenom'])) {
            $data['contact_prenom'] = '';
        }

        if ($schema->hasColumn('clients', 'pays') && !isset($data['pays'])) {
            $data['pays'] = 'Cote dIvoire';
        }
    ...
    private function generateClientCode()
    {
        ...
        if ($lastClient) {
            $lastSequence = intval(substr($lastClient->code, -4));
    ...
    public function dashboard()
    {
        ...
        try {
            ...
            $contractReferenceColumn = $this->getFirstExistingColumn('contrats', ['reference', 'numero'], 'numero');
            $contractAmountColumn = $this->getFirstExistingColumn('contrats', ['montant', 'montant_ttc', 'montant_ht'], 'montant_ttc');
            $quoteReferenceColumn = $this->getFirstExistingColumn('devis', ['reference', 'numero'], 'reference');
            $quoteAmountColumn = $this->getFirstExistingColumn('devis', ['montant', 'total_ttc', 'montant_ttc', 'montant_ht'], 'total_ttc');
            ...
            $recentContracts = $this->normalizeDashboardRows(
                $recentContracts,
                $contractReferenceColumn,
                $contractAmountColumn,
            );
            ...
            $pendingQuotes = $this->normalizeDashboardRows(
                $pendingQuotes,
                $quoteReferenceColumn,
                $quoteAmountColumn,
            );
    ...
    public function clientsStore(Request $request)
    {
        ...
        try {
            app(ClientAccountService::class)->ensureClientAccountById((int) $clientId);
        } catch (\Throwable $e) {
            Log::warning('Client account assignment failed after commercial client creation', [
                'client_id' => $clientId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    public function clientsUpdate(Request $request, $id)
    {
        try {
            ...
            // Certains schémas clients n'ont pas de colonne email.
            if ($schema->hasColumn('clients', 'email')) {
                $rules['email'] = 'required|email|unique:clients,email,' . $id;
            } else {
            ...
            Log::error('Commercial clientsUpdate failed', [
                'client_id' => $id,
                'user_id' => Auth::id(),
                'error' => $e->getMessage(),
            ]);

            return back()->withInput()->with('error', 'Erreur lors de la mise à jour du client: ' . $e->getMessage());
        }
    }
    ...
    public function bonsCommandeCreate()
    {
        ...
        // Générer le numéro automatiquement: BC-YYYYMM-XXX
        $yearMonth = date('Ym');
        $prefix = "BC-{$yearMonth}-";

        // Compter le nombre de bons de commande pour ce mois
        $count = DB::table('bons_commande')
            ->where('numero', 'like', $prefix . '%')
            ->count();

        // Générer le numéro avec padding sur 3 chiffres
        $numero = $prefix . str_pad($count + 1, 3, '0', STR_PAD_LEFT);

        return view('commercial.bons-commande.create', compact('clients', 'contrats', 'numero'));
    }
    ...
    public function devisIndex()
    {
        if (!Schema::hasTable('devis')) {
            return view('commercial.devis.index', [
                ...
                'dbError' => 'La table des devis est absente. Exécutez les migrations du module commercial.',
            ]);
        }
        ...
        $stats = [
            ...
            'accepte' => DB::table('devis')->where('statut', 'accepte')->count(),
            'rejete' => DB::table('devis')->where('statut', 'rejete')->count(),
            'total' => DB::table('devis')->count(),
        ];

        return view('commercial.devis.index', compact('devis', 'stats'));
    }
    ...
    public function bonCommandeCreate()
    {
        ...
        $count = DB::table('bon_commandes')
            ->where('reference', 'like', $prefix . '%')
            ->count();

        $reference = $prefix . str_pad((string) ($count + 1), 3, '0', STR_PAD_LEFT);

        return view('commercial.bon-commande.create', compact('clients', 'reference'));
    }

    public function bonCommandeStore(Request $request)
    {
        $validated = $request->validate([
            'reference' => 'required|string|max:255|unique:bon_commandes,reference',
            'client_id' => 'required|exists:clients,id',
            'date_commande' => 'nullable|date',
            'date_livraison_prevue' => 'nullable|date|after_or_equal:date_commande',
            'duration_days' => 'nullable|integer|min:1',
            'montant_total' => 'required|numeric|min:0',
            'statut' => 'required|in:en_attente,valide,livre,annule',
            'notes' => 'nullable|string',
        ]);
        ...
        DB::table('bon_commandes')->insert([
            ...
            'date_commande' => $validated['date_commande'] ?? null,
        ...
        return redirect()->route('commercial.bon-commande.index')
            ->with('success', 'Bon de commande créé avec succès. Vous pouvez maintenant lancer une mission engin depuis cette ligne.');
    }
