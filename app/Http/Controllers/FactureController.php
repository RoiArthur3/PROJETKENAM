...
class FactureController extends Controller
{
    ...
    public function index(Request $request): View
    {
        try {
            ...
            // Statistiques
            $facturesPayeesQuery = Facture::query()->whereIn('statut', ['payee', 'payée', 'paye', 'paid']);
            ...
            $stats = [
                'total' => Facture::count(),
                'payees' => $facturesPayeesQuery->count(),
                'impayees' => $facturesImpayeesQuery->count(),
                'en_attente' => Facture::where('statut', 'en_attente')->count(),
                'montant_total' => Facture::sum('montant_ttc'),
                'montant_paye' => $facturesPayeesQuery->sum('montant_ttc'),
                'montant_impaye' => $facturesImpayeesQuery->sum('montant_ttc'),
            ];
