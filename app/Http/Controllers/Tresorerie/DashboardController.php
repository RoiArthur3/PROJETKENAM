...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Calculer la variation = solde_actuel - solde_initial
        $soldeInitial = \App\Models\Caisse::where('est_active', true)->sum('solde_initial') ?? 0;
        $variation = $soldeTotal - $soldeInitial;
    ...
    public function decaissements()
    {
        ...
        $decaissements = $this->applyDecaissementVisibility(
            DepenseCaisse::with(['caisse', 'createur', 'beneficiaire', 'operation']),
            $user
        )
            ->latest()
            ->get();
    ...
    public function avances()
    {
        ...
        $acompteTerminology = $this->getAcompteTerminology();

        return view('tresorerie.avances', compact('avances', 'acompteTerminology'));
    }
    ...
    public function avancesCreate()
    {
        ...
        return view('tresorerie.avances-create', compact('acompteTerminology'));
    }
    ...
    public function avancesStore(Request $request)
    {
        ...
        return redirect()->route('tresorerie.avances.index')
            ->with('success', $this->getAcompteTerminology()['singular'] . ' enregistré avec succès !');
    }
    ...
    private function getAcompteTerminology(): array
    {
        // Le stockage historique peut conserver "avances" en BDD/route,
        // mais l'affichage utilise un vocabulaire comptable modernisé.
        return [
            'singular' => 'Acompte',
            'plural' => 'Acomptes',
            'outgoing' => 'Acompte versé',
            'incoming' => 'Acompte perçu',
        ];
    }
    ...
    /**
     * Page Bon pour Accord - Affiche les opérations, factures et décaissements en attente de validation
     */
    public function bonPourAccord()
    {
        $user = Auth::user();

        // Récupérer les opérations en attente de validation
        $bonsPourAccordOperations = \App\Models\Operation::where('statut_courant', 'en_attente')
            ->with(['user', 'service'])
            ->orderByDesc('created_at')
            ->get();

        // Récupérer les factures fournisseur en attente de validation
        $bonsPourAccordFactures = \App\Models\FactureFournisseur::where('statut', 'en_attente')
            ->with(['fournisseur'])
            ->orderByDesc('created_at')
            ->get();

        // Récupérer les décaissements en attente de validation
        $bonsPourAccordDecaissements = $this->applyDecaissementVisibility(
            DepenseCaisse::with(['caisse', 'createur', 'beneficiaire'])
                ->where('statut', 'en_attente'),
            $user
        )->orderByDesc('created_at')->get();

        return view('tresorerie.bon-pour-accord', compact(
            'bonsPourAccordOperations',
            'bonsPourAccordFactures',
            'bonsPourAccordDecaissements'
        ));
    }
}
