...
class DepenseController extends Controller
{
    ...
    public function create(ApprovisionnementCaisse $approvisionnement)
    {
        ...
        // Récupérer les opérations pour lier à la dépense
        $operations = \App\Models\Operation::select('id', 'numero_operation', 'titre', 'montant_estime', 'client_id')
            ->with('client:id,name')
            ->where('statut_courant', 'Approuvé_en_attente_paiement')
            ->orderBy('created_at', 'desc')
            ->get();

        return view('tresorerie.depenses.create', compact('approvisionnement', 'comptes', 'operations'));
    }
