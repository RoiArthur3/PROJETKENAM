...
class EncaissementController extends Controller
{
    public function index()
    {
        ...
        return view('tresorerie.encaissements', compact('encaissements', 'clients'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // 3. Reconcile with Accounting (Create Recette)
            Recette::create([
                'encaissement_id' => $encaissement->id,
                'reference' => $reference,
                'libelle' => 'Recette (Encaissement): ' . $validated['type_encaissement'],
                'montant' => $validated['montant'],
                'date_recette' => $validated['date_encaissement'],
                'categorie' => $validated['type_encaissement'],
                'client_id' => $validated['client_id'] ?? null,
                'caisse_id' => $validated['caisse_id'],
                'mode_paiement' => $validated['mode_paiement'],
                'statut' => 'encaissée',
            ...
            \Log::info("✅ Encaissement créé : {$reference} - Montant: {$validated['montant']} FCFA - Caisse: {$validated['caisse_id']}");

            return redirect()->route('tresorerie.encaissements')
                ->with('success', '✅ Encaissement enregistré avec succès : ' . $reference . ' - ' . number_format($validated['montant'], 0, ',', ' ') . ' FCFA');

        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error("❌ Erreur encaissement : " . $e->getMessage() . " | " . $e->getFile() . ":" . $e->getLine());
            \Log::error("Stack trace: " . $e->getTraceAsString());
            return back()->withInput()
                ->with('error', '❌ Erreur lors de l\'enregistrement : ' . $e->getMessage())
                ->with('debug', $e->getTraceAsString());
        }
    }
