...
class ApprovisionnementController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Effets signes:
        // source += -montant, destination += +montant
        if ($caisseSource && $montant > 0 && $caisseSource->solde_actuel < $montant) {
            return back()
                ->withInput()
                ->with('error', 'Solde insuffisant dans la caisse source (' . number_format($caisseSource->solde_actuel, 0, ',', ' ') . ' FCFA).');
        }

        if ($montant < 0 && $caisseDestination->solde_actuel < abs($montant)) {
            return back()
                ->withInput()
                ->with('error', 'Solde insuffisant dans la caisse destination pour annuler cet approvisionnement.');
        }
        ...
        try {
            ...
            if ($caisseSource) {
                ...
                $caisseSource->decrement('solde_actuel', $montant);
            }
            // Créditer la caisse destination
            $caisseDestination->increment('solde_actuel', $montant);
            ...
            $message = $caisseSource
                ? 'Transfert de ' . number_format($montant, 0, ',', ' ') . ' FCFA effectue avec succes.'
                : 'Approvisionnement de ' . number_format($montant, 0, ',', ' ') . ' FCFA enregistre dans la caisse.';
    ...
    public function decaisser(ApprovisionnementCaisse $approvisionnement)
    {
        ...
        if ($approvisionnement->montant > 0 && $approvisionnement->source->solde_actuel < $approvisionnement->montant) {
            return back()
                ->with('error', 'Le solde de la caisse source est insuffisant pour effectuer ce décaissement.');
        }

        if ($approvisionnement->montant < 0 && $approvisionnement->destination->solde_actuel < abs($approvisionnement->montant)) {
            return back()
                ->with('error', 'Le solde de la caisse destination est insuffisant pour annuler ce decaissement.');
        }
