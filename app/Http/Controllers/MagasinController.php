...
{
    /**
     * Résout dynamiquement la table produits disponible dans la base.
     */
    private function resolveProductTable(): ?string
    {
    ...
    public function entrees()
    {
        if (!Schema::hasTable('stock_entrees')) {
            return view('admin.magasin.entrees', [
                'entrees' => collect([]),
            ])->with('error', "La table 'stock_entrees' n'existe pas encore. Exécutez les migrations du module magasin.");
        }
    ...
    {
        if (!Schema::hasTable('stock_sorties')) {
            return view('admin.magasin.sorties', [
                'sorties' => collect([]),
            ])->with('error', "La table 'stock_sorties' n'existe pas encore. Exécutez les migrations du module magasin.");
        }

        $productTable = $this->resolveProductTable();

        $query = DB::table('stock_sorties')
            ->leftJoin('users', 'stock_sorties.user_id', '=', 'users.id')
            ->select('stock_sorties.*', 'users.name as user_name');

        if ($productTable === 'produits') {
            $query
                ->leftJoin('produits', 'stock_sorties.produit_id', '=', 'produits.id')
                ->addSelect('produits.designation as produit_nom');
        } elseif ($productTable === 'products') {
            $query
                ->leftJoin('products', 'stock_sorties.produit_id', '=', 'products.id')
                ->addSelect(DB::raw('products.name as produit_nom'));
        } else {
            $query->addSelect(DB::raw("'Produit inconnu' as produit_nom"));
        }
    ...
    public function createEntree()
    {
        ...
        if (!$productTable) {
            return redirect()->route('magasin.entrees')
                ->with('error', "Aucune table produit n'a été trouvée (ni 'produits' ni 'products').");
        }
        ...
        if ($productTable === 'produits') {
            ...
            $productsQuery = DB::table('products')
                ->select([
                    'products.id',
                    DB::raw('products.name as designation'),
                    DB::raw('products.reference as code'),
                    DB::raw('products.unit_price as prix_unitaire'),
                ]);

            if (Schema::hasTable('stock_levels')) {
                $productsQuery
                    ->leftJoin(DB::raw('(SELECT product_id, SUM(current_stock) AS stock_actuel FROM stock_levels GROUP BY product_id) stock_sums'), 'stock_sums.product_id', '=', 'products.id')
                    ->addSelect(DB::raw('COALESCE(stock_sums.stock_actuel, 0) as stock_actuel'));
            } else {
                $productsQuery->addSelect(DB::raw('0 as stock_actuel'));
            }

            $produits = $productsQuery
    ...
    public function storeEntree(Request $request)
    {
        ...
        $validated = $request->validate([
            'produit_id' => 'required|exists:' . $productTable . ',id',
        ...
        try {
            ...
            if ($productTable === 'produits' && Schema::hasColumn('produits', 'stock_actuel')) {
                DB::table('produits')
            ...
            if (!empty($validated['prix_unitaire'])) {
                if ($productTable === 'produits' && Schema::hasColumn('produits', 'prix_unitaire')) {
                    DB::table('produits')
                        ->where('id', $validated['produit_id'])
                        ->update(['prix_unitaire' => $validated['prix_unitaire']]);
                }

                if ($productTable === 'products' && Schema::hasColumn('products', 'unit_price')) {
                    DB::table('products')
                        ->where('id', $validated['produit_id'])
                        ->update(['unit_price' => $validated['prix_unitaire']]);
                }
            }
    ...
    public function createSortie()
    {
        ...
        if (!$productTable) {
            return redirect()->route('magasin.sorties')
                ->with('error', "Aucune table produit n'a été trouvée (ni 'produits' ni 'products').");
        }
        ...
        if ($productTable === 'produits') {
            $produits = DB::table('produits')
                ->select([
                    ...
                    'prix_unitaire',
                    'stock_actuel',
                    DB::raw("COALESCE(unite, 'Unité') as unite"),
                    DB::raw('COALESCE(stock_min, 0) as stock_min'),
                ])
            ...
            $productsQuery = DB::table('products')
                ->select([
                    ...
                    DB::raw('products.reference as code'),
                    DB::raw('products.unit_price as prix_unitaire'),
                    DB::raw("COALESCE(products.unit, 'Unité') as unite"),
                    DB::raw('0 as stock_min'),
                ]);
            ...
            if (Schema::hasTable('stock_levels')) {
                $productsQuery
                    ->leftJoin(DB::raw('(SELECT product_id, SUM(current_stock) AS stock_actuel FROM stock_levels GROUP BY product_id) stock_sums'), 'stock_sums.product_id', '=', 'products.id')
                    ->addSelect(DB::raw('COALESCE(stock_sums.stock_actuel, 0) as stock_actuel'))
                    ->whereRaw('COALESCE(stock_sums.stock_actuel, 0) > 0');
            } else {
                $productsQuery
                    ->addSelect(DB::raw('0 as stock_actuel'))
                    ->whereRaw('1 = 0');
            }
    ...
    public function storeSortie(Request $request)
    {
        ...
        if ($productTable === 'produits') {
            ...
            if (!$produit || $produit->stock_actuel < $validated['quantite']) {
                return back()->withInput()->with('error', 'Stock insuffisant');
            }
        } elseif ($productTable === 'products') {
            $stockActuel = 0;
            if (Schema::hasTable('stock_levels')) {
                $stockActuel = (float) DB::table('stock_levels')
                    ->where('product_id', $validated['produit_id'])
                    ->sum('current_stock');
            }

            if ($stockActuel < (float) $validated['quantite']) {
                return back()->withInput()->with('error', 'Stock insuffisant');
            }
        }
        ...
        try {
            ...
            if ($productTable === 'products' && Schema::hasTable('stock_levels')) {
                DB::table('stock_levels')
                    ->where('product_id', $validated['produit_id'])
                    ->decrement('current_stock', $validated['quantite']);
            }
    ...
    public function editSortie($id)
    {
        ...
        if ($productTable === 'produits') {
            ...
            $produits = DB::table('products')
                ->select([
                    'id',
                    DB::raw('name as designation'),
                    DB::raw('reference as code'),
                ])
                ->orderBy('name')
                ->get();
        } else {
