                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="numero" name="numero"
                                           value="{{ $nextNumero ?? 'FAC-' . date('Y-m-d') . '-001' }}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="bon_commande_id" name="bon_commande_id">
                                        ...
                                        @if(isset($bonsCommande))
                                            @foreach($bonsCommande as $bc)
                                                <option value="{{ $bc->id }}">
                                                    {{ $bc->numero_bc ?? 'BC-' . $bc->id }} - {{ $bc->reference ?? '' }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
...
$(document).ready(function() {
    ...
    $('#factureForm').submit(function(e) {
        ...
        $.ajax({
            url: '{{ route("comptabilite.factures.store") }}',
            method: 'POST',
            data: formData,
            success: function(response) {
                if(response.success) {
                    alert('✅ ' + response.message);
                    window.location.href = response.redirect || '{{ route("comptabilite.factures.index") }}';
                } else {
                    alert('❌ ' + response.message);
                }
            },
