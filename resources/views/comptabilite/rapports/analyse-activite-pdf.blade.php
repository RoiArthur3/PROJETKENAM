...
<head>
    <meta charset="UTF-8">
    <title>ANALYSE DE L'ACTIVITE - SIG/CAF</title>
    <style>
...
<body>
    <h2>ANALYSE DE L'ACTIVITE - SIG et CAF</h2>
    ...
    <table>
        ...
        <tbody>
            @foreach($donnees['lignes'] as $ligne)
                ...
                @endphp
                <tr class="{{ in_array($ligne['key'], ['ebe', 'resultat_net', 'caf']) ? 'strong' : '' }}">
                    <td>{{ $ligne['libelle'] }}</td>
                    <td class="num">{{ number_format($n1, 0, ',', ' ') }}</td>
                    <td class="num">{{ number_format($n, 0, ',', ' ') }}</td>
                    <td class="num">{{ $variation >= 0 ? '+' : '' }}{{ number_format($variation, 0, ',', ' ') }}</td>
                    <td class="num">
