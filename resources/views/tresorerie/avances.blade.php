...
@section('title', (($acompteTerminology['plural'] ?? 'Acomptes')) . ' - KENAM SERVICES')

@section('content')
<div class="content-wrapper">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0">Gestion des {{ $acompteTerminology['plural'] ?? 'Acomptes' }}</h1>
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('tresorerie.avances.create') }}" class="btn btn-primary">
                <i class="fas fa-plus me-2"></i>Nouveau {{ strtolower($acompteTerminology['singular'] ?? 'Acompte') }}
            </a>
        </div>
    </div>
            ...
            <div class="card border-left-primary shadow h-100 py-2">
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">Total {{ $acompteTerminology['plural'] ?? 'Acomptes' }}</div>
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title mb-4">Liste des {{ $acompteTerminology['plural'] ?? 'Acomptes' }}</h5>

            <!-- Filtres -->
            <div class="row mb-3">
                <div class="col-md-3">
                    <input type="text" class="form-control" placeholder="Rechercher un acompte..." id="searchInput">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <button type="button" class="btn btn-outline-info" onclick="exportAcomptes()">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <form action="{{ route('tresorerie.avances.destroy', $avance->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cet acompte ?')">
                                        @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function exportAcomptes() {
        ...
        console.log('Acomptes a exporter:', avances);
        alert('Export des acomptes simule (voir console)');
    }

    // Écouteurs d'événements
    document.getElementById('searchInput').addEventListener('input', filterAcomptes);
    document.getElementById('statutFilter').addEventListener('change', filterAcomptes);
    document.getElementById('beneficiaireFilter').addEventListener('change', filterAcomptes);
    document.getElementById('motifFilter').addEventListener('change', filterAcomptes);
});
</script>
