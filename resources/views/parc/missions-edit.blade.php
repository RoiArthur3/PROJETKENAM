...
@section('title', 'Modifier Projet de location Engin / Véhicule | KENAM SERVICES')
        ...
        <div class="col-lg-10">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-warning text-dark py-3">
                    <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Modifier le projet de location {{ $mission->reference }}</h5>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('materiel.missions.update', $mission) }}" method="POST" id="missionForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="source_type" class="form-select">
                                    ...
                                    <option value="prospection" {{ old('source_type', $mission->source_type) === 'prospection' ? 'selected' : '' }}>Prospection</option>
                                    <option value="demande_recherche" {{ old('source_type', $mission->source_type) === 'demande_recherche' ? 'selected' : '' }}>Demande de recherche</option>
                                    <option value="bon_commande" {{ old('source_type', $mission->source_type) === 'bon_commande' ? 'selected' : '' }}>Bon de commande client</option>
                                    <option value="project" {{ old('source_type', $mission->source_type) === 'project' ? 'selected' : '' }}>Projet de location</option>
                                    <option value="other" {{ old('source_type', $mission->source_type) === 'other' ? 'selected' : '' }}>Autre</option>
                                </select>
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label fw-bold">Référence source</label>
                                <input type="text" name="source_reference" class="form-control" value="{{ old('source_reference', $mission->source_reference) }}" placeholder="Ex: BC-202603-001">
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label fw-bold">ID source</label>
                                <input type="number" name="source_id" class="form-control" value="{{ old('source_id', $mission->source_id) }}" min="1" placeholder="Optionnel si document déjà saisi">
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="vehicle_id" class="form-select select2" required id="vehicle_select">
                                    ...
                                    @foreach($vehicles as $v)
                                        <option value="{{ $v->id }}" data-price="{{ $v->prix_location }}" data-type-materiel="{{ strtolower($v->type_materiel ?? '') }}" {{ (string) old('vehicle_id', $mission->vehicle_id) === (string) $v->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="driver_id" class="form-select select2" required>
                                    ...
                                    @foreach($drivers as $d)
                                        <option value="{{ $d->id }}" {{ (string) old('driver_id', $mission->driver_id) === (string) $d->id ? 'selected' : '' }}>{{ $d->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="status" class="form-select fw-bold">
                                    @foreach($statuses as $status)
                                        <option value="{{ $status }}" {{ old('status', $mission->status) === $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold">Lieu de Destination <span class="text-danger">*</span></label>
                                <input type="text" name="destination" class="form-control" required value="{{ old('destination', $mission->destination) }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label class="form-label fw-bold">Date de début <span class="text-danger">*</span></label>
                                <input type="date" name="start_at" class="form-control" id="start_date" required value="{{ old('start_at', optional($mission->start_at)->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label class="form-label fw-bold">Date de fin <span class="text-danger">*</span></label>
                                <input type="date" name="end_at" class="form-control" id="end_date" required value="{{ old('end_at', optional($mission->end_at)->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label class="form-label fw-bold">Nombre de jours prévus <span class="text-danger">*</span></label>
                                <input type="number" name="duration_days" class="form-control" id="duration" min="1" required value="{{ old('duration_days', $mission->duration_days) }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="pointage_submodule" id="pointage_submodule" class="form-select">
                                    <option value="engin" {{ old('pointage_submodule', $mission->pointage_submodule ?? 'engin') === 'engin' ? 'selected' : '' }}>Standard</option>
                                    <option value="camion_plateau" {{ old('pointage_submodule', $mission->pointage_submodule ?? '') === 'camion_plateau' ? 'selected' : '' }}>Camion Plateau</option>
                                </select>
                            </div>
                            <div class="col-md-3 mb-3">
                                <label class="form-label fw-bold">Type de facturation</label>
                                <select name="billing_mode" id="billing_mode" class="form-select">
                                    <option value="standard" {{ old('billing_mode', $mission->billing_mode ?? 'standard') === 'standard' ? 'selected' : '' }}>Standard</option>
                                    <option value="trip" {{ old('billing_mode', $mission->billing_mode ?? '') === 'trip' ? 'selected' : '' }}>Au voyage</option>
                                    <option value="monthly" {{ old('billing_mode', $mission->billing_mode ?? '') === 'monthly' ? 'selected' : '' }}>Au mois</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                <h6 class="fw-bold border-bottom pb-2 mb-3 text-secondary"><i class="fas fa-tachometer-alt me-1"></i>Kilométrage</h6>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold">KM au départ</label>
                                <input type="number" name="start_km" class="form-control" min="0" value="{{ old('start_km', $mission->start_km) }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold">KM à l'arrivée</label>
                                <input type="number" name="end_km" class="form-control" min="0" value="{{ old('end_km', $mission->end_km) }}">
                            </div>
                        </div>
                                ...
                                <div class="p-3 bg-light rounded h-100 shadow-sm border-start border-danger border-4">
                                    ...
                                    <select name="supplier_id" class="form-select select2 mb-3">
                                        ...
                                        @foreach($suppliers as $s)
                                            <option value="{{ $s->id }}" {{ (string) old('supplier_id', $mission->supplier_id) === (string) $s->id ? 'selected' : '' }}>{{ $s->raison_sociale }}</option>
                                        @endforeach
                                    </select>
                                    <label class="form-label fw-bold">Prix Journalier Fournisseur (FCFA)</label>
                                    <input type="number" name="daily_supplier_price" class="form-control text-danger fw-bold" id="daily_supplier" value="{{ old('daily_supplier_price', $mission->daily_supplier_price) }}" min="0">
                                </div>
                            </div>
                            <div class="col-md-6 mb-3">
                                <div class="p-3 bg-light rounded h-100 shadow-sm border-start border-success border-4">
                                    ...
                                    <select name="client_id" class="form-select select2 mb-3">
                                        ...
                                        @foreach($clients as $c)
                                            <option value="{{ $c->id }}" {{ (string) old('client_id', $mission->client_id) === (string) $c->id ? 'selected' : '' }}>{{ $c->raison_sociale }}</option>
                                        @endforeach
                                    </select>
                                    <label class="form-label fw-bold">Prix Journalier Facturé Client (FCFA)</label>
                                    <input type="number" name="daily_client_price" class="form-control text-success fw-bold" id="daily_client" value="{{ old('daily_client_price', $mission->daily_client_price) }}" min="0">
                                </div>
                            </div>
                        </div>
                        <div class="row mb-4">
                            <div class="col-12">
                                <div class="alert alert-info d-flex justify-content-between align-items-center p-4">
                                    <div>
                                        ...
                                        <p class="mb-0 small opacity-75">Calcul automatique basé sur <span id="duration_text">{{ old('duration_days', $mission->duration_days) }}</span> jours</p>
                                    </div>
                                    <div class="text-end">
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="notes" class="form-control" rows="3" placeholder="Détails supplémentaires sur la mission...">{{ old('notes', $mission->notes) }}</textarea>
                        </div>

                        <div class="d-flex justify-content-between mt-4">
                            <a href="{{ route('materiel.missions.show', $mission) }}" class="btn btn-light border px-4 py-2"><i class="fas fa-arrow-left me-1"></i>Retour</a>
                            <button type="submit" class="btn btn-warning px-5 py-2"><i class="fas fa-save me-1"></i>Mettre à jour le Projet de location</button>
                        </div>
                    </form>
                </div>
            </div>
