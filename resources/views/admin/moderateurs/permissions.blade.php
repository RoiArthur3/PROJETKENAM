...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">
                ...
                Modules actuels : {{ implode(', ', (is_array($moderator->modules) ? $moderator->modules : (json_decode($moderator->modules ?? '[]', true) ?: []))) ?: 'Aucun' }}
            </p>
        </div>
        <div>
    ...
    <form method="POST" action="{{ route('admin.comptes.users.submodules.update', $moderator->id) }}">
        @csrf
        @method('PUT')
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        @php
                            ...
                            $allowedModules = is_array($moderator->modules) ? $moderator->modules : (json_decode($moderator->modules ?? '[]', true) ?: []);
                            $allowedSubmodules = is_array($moderator->submodules) ? $moderator->submodules : (json_decode($moderator->submodules ?? '[]', true) ?: []);
                        @endphp
...
function updateCount(moduleKey) {
    ...
    const countEl = document.getElementById('count_' + moduleKey);
