...
@section('title', 'Modifier le Contrôle - KENAM SERVICES')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Modifier le Contrôle #{{ str_pad($controle->id, 6, '0', STR_PAD_LEFT) }}</h1>
        <a href="{{ route('controleur.show', $controle->id) }}" class="btn btn-secondary">
        ...
        <div class="col-lg-10">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('controleur.update', $controle->id) }}" method="POST" id="controleForm">
                        @csrf
                        @method('PUT')
                        
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="vehicule_id" class="form-label required-field">Véhicule</label>
                                <select class="form-select @error('vehicule_id') is-invalid @enderror" id="vehicule_id" name="vehicule_id" required>
                                    <option value="">Sélectionner un véhicule</option>
                                    @foreach($vehicules as $vehicule)
                                        <option value="{{ $vehicule->id }}" {{ old('vehicule_id', $controle->vehicule_id) == $vehicule->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="type_controle" class="form-label required-field">Type de contrôle</label>
                                <select class="form-select @error('type_controle') is-invalid @enderror" id="type_controle" name="type_controle" required>
                                    <option value="technique" {{ old('type_controle', $controle->type_controle) == 'technique' ? 'selected' : '' }}>Contrôle technique</option>
                                    <option value="securite" {{ old('type_controle', $controle->type_controle) == 'securite' ? 'selected' : '' }}>Contrôle de sécurité</option>
                                    <option value="entretien" {{ old('type_controle', $controle->type_controle) == 'entretien' ? 'selected' : '' }}>Contrôle d'entretien</option>
                                    <option value="autre" {{ old('type_controle', $controle->type_controle) == 'autre' ? 'selected' : '' }}>Autre type de contrôle</option>
                                </select>
                                @error('type_controle')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label for="date_controle" class="form-label required-field">Date du contrôle</label>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control @error('kilometrage') is-invalid @enderror" 
                                       ...
                                       value="{{ old('kilometrage', $controle->kilometrage) }}" min="0">
                                @error('kilometrage')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="date" class="form-control @error('prochain_controle') is-invalid @enderror" 
                                       ...
                                       value="{{ old('prochain_controle', $controle->prochain_controle ? $controle->prochain_controle->format('Y-m-d') : '') }}">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="resultat" id="resultat_conforme" value="conforme" 
                                            {{ old('resultat', $controle->resultat) == 'conforme' ? 'checked' : '' }} required>
                                ...
                                <div class="col-md-4">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="resultat" id="resultat_non_conforme" value="non_conforme" 
                                            {{ old('resultat', $controle->resultat) == 'non_conforme' ? 'checked' : '' }}>
                                ...
                                <div class="col-md-4">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="resultat" id="resultat_en_attente" value="en_attente" 
                                            {{ old('resultat', $controle->resultat) == 'en_attente' ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('commentaire') is-invalid @enderror" 
                                     ...
                                     name="commentaire" 
                                     rows="3" 
                                     placeholder="Détails sur le contrôle, observations...">{{ old('commentaire', $controle->commentaire) }}</textarea>
                            @error('commentaire')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end border-top pt-4">
                            <a href="{{ route('controleur.show', $controle->id) }}" class="btn btn-outline-secondary me-2">
