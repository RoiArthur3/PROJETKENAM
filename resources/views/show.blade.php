...
<div class="container-fluid">
    ...
    <div class="mb-4">
        ...
        @if($authUser && (in_array($authUser->role, ['admin', 'superadmin']) || ($authUser->id == $operation->user_id && $operation->statut_courant === 'brouillon')) && !in_array($operation->statut_courant, ['approuvee', 'terminee']))
            <form action="{{ route('operations.destroy', $operation) }}" method="POST" class="d-inline ms-2" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette opération #{{ str_pad($operation->id, 5, '0', STR_PAD_LEFT) }} ? Cette action est irréversible.');">
                @csrf
                @method('DELETE')
